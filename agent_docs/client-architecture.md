...
2. If `changeKey` changed, sets `currentFrontmatter` from response, calls `render(md)` which:
   ...
   - Parses markdown via `marked.parse()` (GFM mode) and passes the result through `sanitizeHtml()` (`utils.js`) before it touches `innerHTML` — marked runs with raw HTML enabled (authored `<details>`, `<p align>`, inline SVG are wanted), so a hostile `.md` must not run script on the origin that owns `/api/save` and `/api/shutdown`. DOMPurify 3.4.15 (pinned + SRI in `template.py`) does the work with `_PURIFY_OPTS` (html+svg profiles, `style`/`form` forbidden, `target` allowed, `file:`/`obsidian:`/`vscode:` schemes allowed); when the CDN is unreachable `_fallbackSanitize` parses into an inert `<template>` and strips `<script>`/`<iframe>`/`<object>`/`<embed>`/`<style>`/`<form>`, the SVG SMIL animators and mXSS carriers (`math`, `noscript`, `xmp`, `template`…), `on*` handlers, and `javascript:`/`data:` URLs (except `data:image/` on `<img>`), re-running until the serialised output is a fixpoint (else escaped as text), warning once. Pre-existing sidecar hardening from the same review: `renderAnnotations` escapes `ann.id`/`ann.type` and uses `CSS.escape` in its selectors, since a sidecar travels with the document. Kept either way: id/class (TOC, footnotes), `data-*`/`aria-*`, task-list checkboxes, `style`. The same call wraps every other `marked.parse` sink (`diff.js` compare, `home.js` card previews, `variables.js` schema tooltips) — a new sink goes through it too; phase15 V12 greps for it
