...
- **Hostile markdown stays inert**—every render passes through DOMPurify (pinned, SRI-checked, with an offline fallback), files served from a document's folder are `nosniff` and anything a browser would script (html, svg, xml) renders sandboxed, and cross-site GETs to the API are refused alongside the existing POST origin check
