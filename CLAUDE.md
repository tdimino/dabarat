...
## Structure
- 11 Python modules in `dabarat/` — `server.py` (HTTP + 44 endpoints), `template.py` (HTML assembly), `annotations.py`, `bookmarks.py`, `frontmatter.py`, `diff.py`, `history.py`, `recent.py`, `workspace.py`, `pdf_export.py`, `__main__.py` (CLI entry)
...
- **Whisper tier (contrast waiver)**: `--ctp-overlay1` text (card meta, figcaptions, footnote backrefs, list markers) is deliberately de-emphasized and waived from WCAG AA by decision — do not "fix" it to 4.5:1. `scripts/color-audit/audit.py` tracks these rows in a separate `waived` section; everything else must pass its tier, and the script exits 1 on any P0.
