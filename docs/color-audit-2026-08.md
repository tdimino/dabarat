...
**Bugs closed**: `--ctp-crust-rgb` was consumed in three box-shadows and defined nowhere (the whole declaration dropped silently for months — `undefined_var_findings` now catches this class); the theme-toggle moon/sun swap and knob were Latte-only / hard white (now all four light themes, `--card-bg`); Vellum was missing from three "surface0 too heavy" groups; `_custom` themes inherited Mocha's `--card-bg`, `--hljs-comment`, and `color-scheme: dark` (now derived from base luminance, with the card/interactive/row-hover tokens emitted; elevation is inherited from `:root`); `THEME_PREVIEW` had no Ink/Vellum swatches and stale Latte/Tokyo Light blues, `SURFACE_COLORS` and the Vibrant fallbacks were hand copies — all three tables deleted in favour of computed style.
...
**Audit engine**: `hardcoded_rgba_findings` scans 3/6-digit hex and named colors across every CSS module plus `palette.js`/`theme.js` (exemptions: the neutral channels `0,0,0` / `255,255,255` / `128,128,128` and Vellum's umber anywhere, print-context white, `mask-image` data URIs, and the hex fallback / contrast-math lines in `theme.js`; neutral *box-shadows* outside `theme-variables.css` are policed separately by phase17 V1); `undefined_var_findings` resolves every `var(--x)` against every theme; `shots.py` captures instance-menu, palette, version-panel, and editor states per theme via CDP.

**Home ground wash (same day, after Tom previewed Latte)**: white cards on Latte's bare `#eff1f5` read as a slab. `.home-screen` now paints two soft radial washes in the theme's own accents over base (blue .07 top-left, mauve .05 bottom-right) — no new hex, every theme keeps its identity, dark themes gain a faint depth glow — and the four light themes' `.home-card` hairline is `rgba(blue, .16)` instead of the grey surface1 ring. The audit gained three `(wash)` rows (`home title`, `home meta`, `home control` on `[BASE, blue .07]`); Rosé Pine's `--home-meta`/`--home-control` landed inside the P3 margin there and were re-solved (`#9692ac` / `#9691af`). Totals unchanged: P0 = 0 · P1 = 0 · P2 = 42 · P3 = 9 · waived = 36.

**Inline code on light themes (same day)**: the `--ctp-surface0` pill (Latte `#ccd0da`, L* 83 on a L* 95 page, 12 L* below it) with a dark-teal ink read as a highlighted box, not typographic texture, and its cool hue fought the blue headings. New `--code-bg` role token: dark themes keep surface0; Latte/Dawn/Tokyo Light take a rose wash (`rgba(maroon, .08–.09)` → ≈4 L* below the page) and Vellum a cinnabar wash (`rgba(red, .08)`, the manuscript rubric). Inks re-solved on the harder surface, the blockquote composite (`[BASE, blockquote-bg, code-bg]`, a new P1 row): Latte `#a5303f`, Dawn `#974960`, Tokyo Light `#843c4a`, Vellum `--ctp-red` as-is. Rose is the split-complement of the ~220° blue heading/link hue; peach-amber was rejected for colliding with the yellow "pending" semantics, mauve for colliding with blockquote italics. Totals unchanged: P0 = 0 · P1 = 0 · P2 = 42 · P3 = 9 · waived = 36.

**Tab bar (same day)**: the bar and every inactive tab painted `--ctp-crust`, the heaviest neutral in each light palette (Latte `#dce0e8`, a cold blue-grey band over a L* 95 page), and on dark themes crust, base, and the tab in between sat within a few L* of each other, so the strip neither separated from the page in light mode nor showed its tabs in dark mode. Light themes now paint the home-ground blue wash (`rgba(blue, .06)`) over `--body-bg` — layered in CSS, since `applyOpacity()` writes `--crust-bg` inline and would clobber a token — with transparent tabs, the `rgba(blue, .16)` card hairline as divider, and the active tab lifted on base; the add-file field moves from surface0 to `--card-bg`. Dark themes become a depth ladder: crust bar, inactive tabs on `--toc-bg` (mantle at the opacity alpha), surface0 hover, base active. A 2px blue top rule on the active tab was tried and rejected. Tab text moves from `--ctp-overlay1` to `--home-control`, registered as `tab inactive (mantle)` for dark, `tab inactive (wash)` for light, plus `tab hover (surface0)` and `tab active` rows — all pass on existing values. Totals unchanged: P0 = 0 · P1 = 0 · P2 = 42 · P3 = 9 · waived = 36.

**Designer's critique pass (same day)**: a GPT-6-Astra review (`.design-critique/20260906-163500__astra-color-front-end-review.md`, local) argued the app should be "quieter, more typographic, and more recognizably its own"; six of its fifteen proposals were adopted, each its own commit. (1) *Metadata chips*: type/model/version were green/blue/mauve on home cards and blue/green/teal in the frontmatter bar — the same field changing hue between surfaces. Both now use one neutral chip (`--badge-neutral-fg` on an overlay1 `.15` wash, new `badge neutral .15` and `fm chip neutral .15 (base)` rows); status keeps peach (awareness), lineage links keep pink (navigable). The indicator rail's accent glow was removed in the same commit and restored an hour later — without it the bar blends into the document (Tom's call). (2) *Blockquotes* lost fill, radius, colored ink, and blanket italic — upright body text with one 2px surface1 rule; `--blockquote-bg` and the `blockquote em, blockquote code { font-style: normal }` workaround are deleted; `--blockquote-color` defaults to text and only Ink/Vellum tint it, so the `blockquote` row audits on bare base and the inline-code-in-blockquote composite row is retired. (3) *Motion*: the status dot no longer pulses, the theme toggle crossfades instead of the 400ms circular reveal, and images/home cards no longer lift on hover. (4) *Scholar's Codex pigments*: new `--home-title` role token (lavender default; tungsten on Ink, iron-gall on Vellum, 3:1 on the wash), h3 rubric ochre on Ink (`#d47a56`, lifted from peach for 5.5:1 headroom) and cinnabar on Vellum; Ink's TOC mark, row hover, and selection go blue like every other theme — pigment is identity, blue is function. (5) *Tables*: authored header casing, neutral 600 headers (`table header` row now `--ctp-text` on mantle), horizontal rules only; grid, zebra, and hover survive as `table.dense`, applied by `render.js` past 8 rows or 5 columns. (6) *Editor toolbar* moves from crust to `--toc-bg` (mantle), 32px buttons with a neutral hover wash and a single blue active wash, "Editing" badge neutral until `body.edit-dirty`; the editor rows re-audit on mantle. Totals unchanged: P0 = 0 · P1 = 0 · P2 = 42 · P3 = 9 · waived = 36.

**Reinstated (minutes later)**: Tom reversed the four color changes on first sight ("it looks like shit now"), then asked for them back — commit e80577c reinstates neutral chips, quotation blockquotes, editorial tables, and the mantle toolbar; the frontmatter indicator glow stays. Totals unchanged.

**Home page critique (2026-09-07)**: a `/design-critique` pass on the home page across all eight themes (`.design-critique/20260906-200000__home-page.md`, local; 23/40, no contrast findings) read the colour system as three families running at once — the title pigment, an always-blue functional accent, and the nine-hue badge vocabulary — with Vellum the only theme where the card strip and the title agreed in hue, and Ink the theme where twelve blue strips under a tungsten title read as foreign. The strip now takes the file kind's accent (`_fileBadges[].hue`, the same colour as the badge wash and the new 6px rail dot) and `--home-title` otherwise, so blue is left with two jobs (function, and the readme/plan kinds). The rail's Recent rows drop the identical green MD glyph and the repeating badge chip for the dot, and hover moves from raw surface0 to `--interactive-muted-bg` — on Vellum surface0 is lighter than crust (Addendum 1's depth inversion) and a hovered row floated above the rail. Metadata sizes below 11px (9/10px) move to 10.5px tabular mono; no token values changed and no rows were added, since every affected pair (`--home-meta`/`--home-control`/`--ctp-subtext1` on mantle, `--badge-*-fg` over their washes) was already registered. Totals unchanged: P0 = 0 · P1 = 0 · P2 = 42 · P3 = 9 · waived = 36.
