...
**Reversal (same evening)**: Tom judged the neutralized result worse ("it looks like shit now") and the four color changes were reverted in one commit — metadata chips back to their per-field hues on card and bar, blockquotes back to the mauve rule + mantle wash + italic with the `em`/`code` upright reset, tables back to uppercase blue headers with grid + zebra + hover everywhere, the editor toolbar back to crust with the glow hover and yellow "Editing" badge; `--blockquote-bg` and its audit composite rows are restored. Kept from the critique pass: the motion cut (static dot, crossfade, no hover lift) and the Scholar's Codex pigment jobs (`--home-title`, Ink ochre / Vellum cinnabar h3). Lesson recorded in memory: a reviewer's "quieter" is not a design direction Tom has approved — signature hues and effects are identity, not noise.
